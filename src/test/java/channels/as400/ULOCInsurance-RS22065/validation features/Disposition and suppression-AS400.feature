@Dispositions_Validation_2423
Feature: Validate Creditor Insurance-2423 Disposition and Suppression test cases
...
#  Book appointment(ALL,DEL,ACT), Customer self service(ALL,DEL,ACT) customer defer(DEC)
  @CreditorInsurance @CreditorInsurance_DispositionSuppression @2423 @AS400 @VS17084
  Scenario Outline: Creditor Insurance_VS17084_2423_AS400 Validate <TestID> suppression logic for dispositions
...
#    request (Karate keyword) for sending request string
  ...
  #    Verifying Success with status code 200
    ...
    * print '***************Response after disposition*************'+karate.pretty(afterDispositionResponseString)
    ...
    Then match null == karate.get('//Offers/NBACode[contains(text(),'+offerCode+') ]/preceding-sibling::ID/text()')

    # Fetch recorded activity for CID from IH
    And call read('classpath:channels/as400/ReusableScenarios.feature@fetchIHRows')

    * call read('classpath:channels/as400/ReusableScenarios.feature@CreditorInsuranceDispositionOfferValidationWithIH')
  ...
  @CreditorInsurance @CreditorInsurance_DispositionDisplay @2423 @AS400 @VS17084
  Scenario Outline: Creditor Insurance_VS17084_2423_AS400 Validate <TestID> display logic for dispositions
    * def testDataFileName = CreditorInsurancetestDataFileName
    # Reading data from Excel sheet
...
#    request (Karate keyword) for sending request string
  ...
  #    Verifying Success with status code 200
    ...
    * match cid == karate.get('//Offers/NBACode[contains(text(),'+offerCode+') ]/preceding-sibling::ID/text()')
