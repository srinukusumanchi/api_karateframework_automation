...
Feature: Create test data for NOVA - GIC Renewal Investments Disposition and Suppression test cases
  ...
  @GICTestData_NOVA @GIC_NOVA_DispositionSuppression_TestCases_TestData @GIC_NOVA_DispositionSuppressionTestCases_4587 @4587  @NOVA @ET14061
  Scenario Outline: GIC Renewal Investments_NOVA Create test data for Disposition and Suppression Test Cases Of 4587
    ...
    * call read('classpath:channels/nova/ReusableScenarios.feature@ACCT_DIM_Insertion_Database')
    ...
    Examples:
      | TestID                                                                                                                                                         | MaturityDate |
      | "4587_Perform Accept Completed Disposition and offer should display when Lead scope - ACT"                                                                     | "21"         |
      | "4587_Perform Viewed Disposition and offer should display when Lead scope - ACT"                                                                               | "21"         |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=ALL then offer should not display"        | "21"         |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=DEC then offer should not display"        | "21"         |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=TP3 or ACT then offer should not display" | "21"         |
      | "4587_Perform Viewed(VWD) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=TP3 or ACT then offer should display"               | "21"         |
      | "4587_Perform Viewed(VWD)  disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=DEC then offer should not display"                 | "21"         |
      | "4587_Perform Viewed(VWD) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=ALL then offer should display"                      | "21"         |
