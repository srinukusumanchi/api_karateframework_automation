Feature: Validate GIC Renewal-4587 Cross kill Disposition and Suppression test cases
  ...
  @GICRenewal @GICRenewal_CrossKillDispositionSuppression_4587 @4587 @NOVA @ET14061
  Scenario Outline: GIC Renewal_ET14061_4587_NOVA Validate <TestID> suppression logic for cross kill dispositions
...
#    Printing request body in console
  ...
  #    Verifying Success with status code 200
    ...
    * assert getLength(failedResults) == 0
    ...
    Examples:
      | TestID                                                                                                                                                         |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=ALL then offer should not display"        |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=DEC then offer should not display"        |
      | "4587_Perform Accept Completed(ATC) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=TP3 or ACT then offer should not display" |
      | "4587_Perform Viewed(VWD)  disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=DEC then offer should not display"                 |




  @GICRenewal @GICRenewal_CrossKillDispositionDisplay_4587 @4587 @NOVA @ET14061
  Scenario Outline: GIC Renewal_ET14061_4587_NOVA Validate <TestID> display logic for cross kill dispositions
    * def testDataFileName = GICtestDataFileName
    # Reading data from Excel sheet
...
#    Checking offer displayed in SOAP
    ...
    * assert getLength(failedResults) == 0
    ...
    Examples:
      | TestID                                                                                                                                           |
      | "4587_Perform Viewed(VWD) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=TP3 or ACT then offer should display" |
      | "4587_Perform Viewed(VWD) disposition in NOVA Channel and give a SOAP call in AS400 channel with LeadScope=ALL then offer should display"        |
