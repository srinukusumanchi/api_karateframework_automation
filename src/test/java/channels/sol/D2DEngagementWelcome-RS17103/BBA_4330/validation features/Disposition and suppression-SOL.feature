@CIEPROG-50  @Dispositions_Validation_4330
Feature: Validate D2D Engagement Welcome-BBA-4330 Disposition and Suppression test cases
...
#Accepted Complete (ATC), Deleted (DEL)
  @D2DWelcome @D2DEngagementWelcomeBBA_ATC_DEL_Disposition @4330 @SOL @RS17103 @TOTAL#02
  Scenario Outline: D2D Engagement Welcome-BBA_RS17103_4330_SOL Validate <TestID> accept,delete disposition & suppression logic
...
#    request (Karate keyword) for sending request string
  ...
  #    Verifying Success with status code 200
    ...
    * assert getLength(failedResults) == 0
    Examples:
      | TestID                                                         |
      | "4330_Perform Accept Disposition and offer should not display" |
      | "4330_Perform Delete Disposition and offer should not display" |



    # Viewed(VWD), Reminder Me Later(RML), No Thank You (NTY), Accepted in Progress (ATP)
  @D2DWelcome @D2DEngagementWelcomeBBA_VWD_RML_NTY_ATP_Disposition @4330 @SOL @RS17103 @TOTAL#04
  Scenario Outline: D2D Engagement Welcome-BBA_RS17103_4330_SOL Validate <TestID> view or remind me later or no thank you or accept in progress disposition & suppression logic
    # Reading data from Excel sheet
    * def testDataFileName = WelcometestDataFileName
...
#    request (Karate keyword) for sending request string
  ...
  #    Verifying Success with status code 200
    ...
    * assert getLength(failedResults) == 0
    Examples:
      | TestID                                                                   |
      | "4330_Perform Viewed Disposition and offer should display"               |
      | "4330_Perform Remind me later Disposition and offer should display"      |
      | "4330_Perform Accepted In Progress Disposition and offer should display" |
      | "4330_Perform No Thank You Disposition and offer should display"         |