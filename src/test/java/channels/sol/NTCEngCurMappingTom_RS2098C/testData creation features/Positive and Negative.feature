@NTCMappingTomorrowIHPosNeg_TestData_5151
Feature: Create test data for NTC Engagement Curriculum - Mapping Tomorrow IH, Positive Anad Negative test cases
  ...
  @NTCEngagementMappingTomorrowTestData_PosTcs @5151 @SOL @RS2098C
  Scenario Outline: D2D Engagement Curiculum - Mapping Tomorrwo SOL Create test data for Positive Test Cases
...
#      | "5151_Account type is opened =181 days"                       |
      | "5151_Account type is opened=210 days"                        |
#      | "5151_Account type is opened =200 days"                       |
#      | "5151_Validate IH Record fields against Proposition"          |


  @NTCMappingTomorrow_Negative_TestData @5151 @SOL @RS2098C
  Scenario Outline: D2D Engagement NTC Mapping Tomorrow Create test data for Negative Test cases
  #    Reading testId from Example section
    * def testId = <TestID>
    * def testDataFileName = NTC5151NegTestDataFileName
    # Generate Random cid to insert into database
    * def CID = randGen(15)
    * print  "Random generated CID is "+CID
  ...
  # insert the created Account Number into the ACCT_DIM sheet
    ...
    * call read('classpath:channels/sol/ReusableScenarios.feature@ACCT_DIM_Insertion_Database')
    Examples:
      ...
      | "N-5151_Account Open Date equal to Null"    |